...
def create_pdf(fields, table_data, signature_img=None):
    ...
    if signature_img:
        try:
            can.drawImage(signature_img, 80, 50, width=100, height=50, mask=[0,255,255,255,255,255])
...
with st.form(key='table_form', clear_on_submit=False):
    ...
    if submit_button:
        ...
        signature_img = signature(st_canvas(update_streamlit=True, stroke_color= "red", background_color='white', key="canvas"))
